    ...
    public static partial class Ddb_Intro
    {
        ...
        public static async Task<bool> ReadingMovie_async(int year, string title, bool report)
        {
            ...
            try
            {
                ...
                //Task<Document> lerFilme = client
                string query = @"SELECT * FROM ""Movies"" WHERE ""year"" = 2013 AND ""title"" = 'Turn It Down, Or Else!'";
                var result = client.ExecuteStatement(new ExecuteStatementRequest()
                {
                    Statement = query
                });
